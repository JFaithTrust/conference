...
interface TableProps<D, V> {
  ...
  data: D[]
  hasFilter?: boolean
  hasSearchbar?: boolean // Add hasSearchbar prop
  ...
  return (
    <div className={clsx("flex flex-col gap-y-[18px] px-[30px]")}>
      <div className={"flex items-center justify-between"}>
        {/* Conditionally render search bar based on hasSearchbar prop */}
        {hasSearchbar && (
              ...
              <TableRow key={row.id} data-state={row.getIsSelected() && "selected"}>
                {row.getVisibleCells().map((cell, cellIndex) => (
                  <TableCell
                    key={cell.id}
                    className={clsx({
                      "min-w-28 flex justify-end items-center": cellIndex === row.getVisibleCells().length - 1,
                    })}
                  >
